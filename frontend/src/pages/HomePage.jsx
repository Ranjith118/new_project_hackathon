...
export default function HomePage() {
  ...
  return (
    <div className="min-h-full bg-[#0A0F1A] -m-4 lg:-m-6 overflow-x-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-10 pb-8">
          ...
          <div className="flex justify-center mb-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-orange-500/10 border border-orange-500/20 rounded-full">
              ...
              <span className="text-[11px] sm:text-xs text-orange-400 font-medium">AI-Powered Industrial Maintenance Platform</span>
            </div>
          </div>
          ...
          <div className="text-center mb-5">
            <div className="flex items-center justify-center gap-3 mb-3">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center shadow-lg shadow-orange-500/30">
            ...
            <p className="text-sm sm:text-lg text-slate-400 mt-2 max-w-2xl mx-auto px-2">
          ...
          <div className="flex justify-center mb-5">
            <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-5 py-2 bg-[#1E293B]/80 border border-[#334155] rounded-full text-xs sm:text-sm">
              <span className="text-slate-500 hidden sm:inline">Steel Plant</span>
              <span className="w-px h-4 bg-[#334155] hidden sm:inline-block" />
          ...
          {/* Live stat cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 max-w-3xl mx-auto mb-5">
            {[
              ...
              <div key={label} className={`rounded-xl border p-2.5 sm:p-3 text-center ${bg}`}>
                <Icon className={`w-4 h-4 ${color} mx-auto mb-1`} />
                <div className={`text-xl sm:text-2xl font-heading font-bold ${color}`}>{val}</div>
          ...
          {/* CTA buttons */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3 px-2">
            <button onClick={() => navigate('/dashboard')}
              className="flex items-center justify-center gap-2 px-5 py-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold rounded-xl shadow-lg shadow-orange-500/30 transition-all">
              <BarChart3 className="w-4 h-4" /> Open Dashboard <ArrowRight className="w-4 h-4" />
            </button>
            <button onClick={() => navigate('/assistant')}
              className="flex items-center justify-center gap-2 px-5 py-3 bg-[#1E293B] border border-purple-500/30 hover:border-purple-500/60 text-purple-400 hover:text-purple-300 font-semibold rounded-xl transition-all">
              <Brain className="w-4 h-4" /> AI Assistant
            </button>
            <button onClick={() => navigate('/intelligence-hub')}
              className="flex items-center justify-center gap-2 px-5 py-3 bg-[#1E293B] border border-[#334155] hover:border-teal-500/40 text-slate-300 hover:text-white font-semibold rounded-xl transition-all">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="text-center mb-4 sm:mb-6">
          <h2 className="text-xl sm:text-2xl font-heading font-bold text-white">Platform Modules</h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-1">12 integrated modules covering the complete maintenance lifecycle</p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
          {MODULES.map(({ icon:Icon, label, sub, href, color, glow }) => (
            <button key={href} onClick={() => navigate(href)}
              className={`group relative p-3 sm:p-4 bg-[#1E293B] border border-[#334155] hover:border-transparent rounded-xl text-left transition-all duration-200 active:scale-[0.98] hover:scale-[1.02] hover:shadow-lg ${glow}`}>
              <div className={`absolute inset-0 rounded-xl bg-gradient-to-br ${color} opacity-0 group-hover:opacity-10 transition-opacity`} />
              <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-gradient-to-br ${color} flex items-center justify-center mb-2 sm:mb-3 shadow-md`}>
                <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
              </div>
              <div className="text-xs sm:text-sm font-semibold text-white leading-tight">{label}</div>
              <div className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5 leading-relaxed hidden sm:block">{sub}</div>
              <ChevronRight className="absolute top-3 right-3 w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-600 group-hover:text-slate-400 transition-colors" />
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-6 sm:pb-8">
        <div className="text-center mb-4 sm:mb-6">
          <h2 className="text-xl sm:text-2xl font-heading font-bold text-white">AI Capabilities</h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-1">Built on production-grade ML models and LLM integration</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
          {FEATURES.map(({ icon:Icon, title, desc, color }) => (
            <div key={title} className="p-3 sm:p-4 bg-[#1E293B] border border-[#334155] rounded-xl hover:border-[#475569] transition-all">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 sm:py-5">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-600 hidden sm:inline">— Steel Manufacturing Plant · AI Maintenance Platform v2.0</span>
            </div>
